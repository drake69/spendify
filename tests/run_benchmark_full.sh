...
#
# Usage:
#   bash tests/run_benchmark_full.sh                           # both phases, 1 run
#   bash tests/run_benchmark_full.sh --runs 3
#   bash tests/run_benchmark_full.sh --benchmark pipeline      # classifier only
#   bash tests/run_benchmark_full.sh --benchmark categorizer
#   bash tests/run_benchmark_full.sh --vllm-url http://gpu:8000/v1
#   bash tests/run_benchmark_full.sh --ollama-url http://192.168.1.5:11434
#   bash tests/run_benchmark_full.sh --skip-llama
#   bash tests/run_benchmark_full.sh --skip-ollama
#   bash tests/run_benchmark_full.sh --skip-vllm
#   bash tests/run_benchmark_full.sh --setup-only             # setup without running
#
...
run_phase() {
    ...
    local script
    [ "$phase" = "pipeline" ]    && script="tests/benchmark_pipeline.py"
    [ "$phase" = "categorizer" ] && script="tests/benchmark_categorizer.py"
    STEP=$((STEP + 1))
    echo ""
    echo "────────────────────────────────────────────────────────────"
...
echo "════════════════════════════════════════════════════════════"
echo ""
echo "  Archive : tests/results_archive/  (versioned per-run CSV)"
echo "  Legacy  : tests/generated_files/benchmark/results_all_runs.csv"
