...
Legge tutti i CSV in tests/results_archive/ (o un file specificato) e produce:
...
Usage:
    python tests/aggregate_results.py                     # legge tutti i CSV in results_archive/
    python tests/aggregate_results.py --csv FILE.csv      # usa un CSV specifico
    python tests/aggregate_results.py --output report.txt # salva report su file
    python tests/aggregate_results.py --predict           # stampa anche previsioni
    python tests/aggregate_results.py --type classifier   # solo classifier o categorizer
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--csv", metavar="FILE",
        help="Usa questo CSV invece dei file in results_archive/"
    )
    parser.add_argument(
        "--output", metavar="FILE",
    ...
    parser.add_argument(
        ...
        help="Elenca i CSV disponibili in results_archive/ ed esci"
    )
    args = parser.parse_args()

    if args.list:
        if not _RESULTS_ARCHIVE_DIR.exists() or not list(_RESULTS_ARCHIVE_DIR.glob("*.csv")):
            print("Nessun CSV in results_archive/. Esegui prima un benchmark.")
            sys.exit(0)
        print("CSV disponibili in results_archive/:")
