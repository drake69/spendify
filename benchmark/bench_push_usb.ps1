...
Write-Host "  1) Copia dalla chiavetta al disco locale:"
Write-Host "     Windows     : powershell -ExecutionPolicy Bypass -File $Dest\benchmark\bench_load_usb.ps1 -From $Dest"
Write-Host "     Linux/macOS : bash $Dest/benchmark/bench_load_usb.sh --from $Dest"
Write-Host "  2) Avvia il benchmark (gia' nella cartella giusta dopo bench_load_usb):"
Write-Host "     Windows     : Set-Location `$env:USERPROFILE\Desktop\spendif-ai; powershell -ExecutionPolicy Bypass -File benchmark\run_benchmark_full.ps1"
Write-Host "     Linux/macOS : cd ~/Desktop/spendif-ai && bash benchmark/run_benchmark_full.sh"
Write-Host "  3) Salva i risultati sulla chiavetta:"
Write-Host "     Windows     : powershell -ExecutionPolicy Bypass -File benchmark\bench_save_usb.ps1 -Dest $Dest"
Write-Host "     Linux/macOS : bash benchmark/bench_save_usb.sh --dest $Dest"
Write-Host ""
Write-Host "Poi [dev] raccogli con:"
