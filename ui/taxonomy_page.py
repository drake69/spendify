...
def _render_section(engine, type_key: str, section_label: str) -> bool:
    ...
    if not cats:
        ...
        for cat in cats:
            subs = cat["subs"]
            with st.expander(f"**{cat['name']}** — {len(subs)} sottocategorie", expanded=False):
                ...
                if st.button("➕ Aggiungi sottocategoria",
                    ...
                    ns = new_sub.strip()
                    existing_names = [s["name"] for s in subs]
                    if not ns:
                        st.warning("Il nome non può essere vuoto.")
                    elif ns in existing_names:
    ...
    if submitted:
        ...
        if not nc:
            ...
            with get_session(engine) as session:
                existing = [c.name for c in get_taxonomy_categories(session, type_filter=type_key)]  # ORM objects inside session — safe
                if nc in existing:
                    st.error(f"Categoria '{nc}' già presente.")
                else:
                    new_cat = create_taxonomy_category(session, nc, type_key)
                    subs_list = [s.strip() for s in new_cat_subs_raw.splitlines() if s.strip()]
                    for idx, sub_name in enumerate(subs_list):
                        create_taxonomy_subcategory(session, new_cat.id, sub_name)
                    session.commit()
...
def render_taxonomy_page(engine):
    ...
    with tab_exp:
        if _render_section(engine, "expense", "Categorie di Spesa"):
            changed = True

    with tab_inc:
        if _render_section(engine, "income", "Categorie di Entrata"):
            changed = True
