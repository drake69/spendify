...
## 1. Mandatory initial configuration

**⚠️ Before performing any import you must complete these two steps:**

| Order | What to configure | Where | Why it is required |
|---|---|---|---|
| 1 | **Account holder names** | Impostazioni → Titolari | Required for transfer detection and PII sanitisation |
| 2 | **At least one bank account** | Impostazioni → Conti bancari | Recommended for stable deduplication across sessions |

If account holder names are not configured, the Import page shows an error and blocks processing.
...
```
[ ] 1. Go to ⚙️ Impostazioni
[ ] 2. Fill in "Nomi titolari" with your name (and variants)
[ ] 3. Add your bank accounts (at least the main ones)
[ ] 4. Choose the LLM backend and configure URL/API key if necessary
[ ] 5. Click "💾 Salva impostazioni"
[ ] 6. Go back to 📥 Import and load the first bank statement
```
