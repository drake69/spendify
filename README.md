...
The `.env` file contains only two infrastructure parameters — everything else (LLM backend, API keys, model, owner names, date format, language…) is configured from the **⚙️ Settings** page and persisted in the DB:
...
# Only needed if using docker compose --profile llama-cpp:
